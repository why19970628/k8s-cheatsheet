...
func GetDeployment(client kubernetes.Interface, namespace string, name string) models.MyDeployment {
...
func ListDeployments(client kubernetes.Interface, namespace string) []models.MyDeployment {
...
func PatchDeploymentEnv(client kubernetes.Interface, namespace string, deploymentName string, env string) error {
...
func DeleteDeploymentEnv(client kubernetes.Interface, namespace string, deploymentName string, env string) error {
	envs := strings.Split(env, ":")
	key := envs[0]
	//value := envs[1]
	gotDeployment, err := client.AppsV1().Deployments(namespace).Get(context.TODO(), deploymentName, metav1.GetOptions{})
